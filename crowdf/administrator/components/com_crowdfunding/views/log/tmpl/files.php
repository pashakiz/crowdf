            ...
            <?php for ($i = 0; $i < count($this->files); $i++) {
                ...
                <tr id="js-file-row<?php echo $i; ?>">
                    ...
                    <td>
                        ...
                        <img src="../media/com_crowdfunding/images/ajax-loader.gif" style="display: none;"
                             id="js-ajaxload-icon<?php echo $i; ?>"/></td>
                    <td class="center nowrap hidden-phone">
                        <a href="javascript: void(0);" class="btn btn-small hasTooltip"
                           title="<?php echo JText::sprintf("COM_CROWDFUNDING_FILE_IS", $this->escape($this->files[$i])); ?>"><i
                                class="icon-question-sign"></i></a>
                        <a href="<?php echo JRoute::_("index.php?option=com_crowdfunding&task=log.remove"); ?>"
                           data-row-id="js-file-row<?php echo $i; ?>"
                           data-filename="<?php echo $this->escape($file); ?>"
                           class="btn btn-small hasTooltip js-log-file-remove-btn"
                           title="<?php echo JText::_("JACTION_DELETE"); ?>"><i class="icon-delete"></i></a>
                        <a href="<?php echo JRoute::_("index.php?option=com_crowdfunding&task=log.download&format=raw&file=" . rawurlencode($file)); ?>"
                           class="btn btn-small hasTooltip"
                           title="<?php echo JText::_("COM_CROWDFUNDING_DOWNLOAD"); ?>"><i
